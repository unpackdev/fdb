...
func (sm *StateManager) WaitForStates(stateType StateType, desiredStates []State, timeout time.Duration) error {
	...
	for {
		currentState := sm.GetState(stateType)
		for _, desiredState := range desiredStates {
			if currentState == desiredState {
				sm.logger.Debug("Service has reached one of the desired states.", zap.String("service_type", string(stateType)), zap.String("state", string(currentState)))
				return nil
			}
		}
		if time.Now().After(deadline) {
			err := fmt.Errorf("service %s did not reach one of the desired states %v within %s", stateType, desiredStates, timeout)
			sm.logger.Error("Service did not reach one of the desired states within the given timeout.", zap.String("service_type", string(stateType)), zap.Any("desired_states", desiredStates), zap.Duration("timeout", timeout))
			return err
		}
